        ...
        implements HttpRequestFilter
{
    ...
    public void handleInternalRequest(ContainerRequestContext request)
    {
        ...
        HashCode signature = HashCode.fromString(getRequiredHeader(request, TRINO_HEADERS.requestInternalSignature()));
        long validUntil = parseLong(getRequiredHeader(request, TRINO_HEADERS.requestInternalRequestValidUntil()));

        if (!signature.equals(signature(nodeId, request.getMethod().toUpperCase(ENGLISH), validUntil, request.getUriInfo().getRequestUri()))) {
    ...
    private HashCode signature(String method, String nodeId, long requestTimestampMillis, URI uri)
    {
        String queryString = firstNonNull(uri.getQuery(), "");
        return hashing
                .newHasher(nodeId.length() + method.length() + uri.getPath().length() + queryString.length() + Long.BYTES)
    ...
    public Request filterRequest(Request request)
    {
        ...
        HashCode signature = signature(nodeId, request.getMethod().toUpperCase(ENGLISH), validUntil, request.getUri());
        return fromRequest(request)
                ...
                .addHeader(TRINO_HEADERS.requestInternalSignature(), signature.toString())
                .build();
    }
